...
describe("statusCodes", function() {
  ...
  describe("/api/produce/:name", function() {
    ...
    it("DELETE returns status code 200 when name is provided", function(done) {
      request.delete(baseUrl+ 'api/produce/peach', function(error, response, body) {
        assert.equal(200, response.statusCode);
        done();
      });
    });
    it("DELETE returns status code 500 when name is NOT provided", function(done) {
      request.delete(apiUrl, function(error, response, body) {
        assert.equal(500, response.statusCode);
        done();
      });
    });
    it("DELETE returns status code 500 when name provided is not in data list", function(done) {
